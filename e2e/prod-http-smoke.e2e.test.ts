...
async function fetchSkillDetail() {
  const response = await fetchWithTimeout(new URL(`/api/v1/skills/${getSkillSlug()}`, getSiteBase()), {
...
describe('prod http smoke', () => {
  ...
  it('serves SSR skill html for a public skill page', async () => {
    ...
    expect(html).toContain(`<link rel="canonical" href="${getSiteBase()}/${owner}/${detail.skill.slug}"/>`)
  ...
  it('serves the skill og image for the latest published version', async () => {
    ...
    const response = await fetchWithTimeout(new URL(`/og/skill.png?${params.toString()}`, getSiteBase()))
